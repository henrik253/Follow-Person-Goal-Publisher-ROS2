...
class ObjectTracker(Node): 
    ...
    def remove_cropped_person_folder(self, custom_id): 
        ...
        if os.path.exists(self.image_storage_dir):
            ...
            #shutil.rmtree(os.path.join(self.image_storage_dir, f"person_{custom_id}"))
            ...
            for id2 in disappeared_ids[i + 1:]:
                ...
                if similarities:
                    ...
                    if max_similarity >= MIN_REMOVE_CANDIDATE_TRESHOLD:   # If similarity is above the threshold, we consider them as the same person or as an inconsistency! 
                        ...
                        ids_to_remove[lower_id] = higher_id
    ...
    # just fill up 
    def merge_candidates(self,from_id, to_id):  
        index = len(self.fixed_tracked_persons[to_id]) 
        while index < MAX_LIST_SIZE_FIXED_FEATURES:
            self.fixed_tracked_persons[to_id] = self.fixed_tracked_persons[from_id].pop() 
            index + 1
        del self.disappeared_persons[from_id]
        del self.fixed_tracked_persons[from_id]
        del self.last_tracked_persons[from_id]
    ...
    def image_callback(self, msg):
            ...
            for bbox, track_id, class_id, confidence, kp in zip(trackedResults[0].boxes, track_ids, class_ids, confidences, keypoints_data):
                ...
                if yolo_id in self.yolo_to_custom_id:
                    ...
                    if best_match_id is not None:
                        ...
                        self.get_logger().info(f'   after removing, disappeared available ids:: {self.disappeared_persons.keys()}')
                ...
                cv2.putText(cv_image, f'ID: {custom_id}', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
                # DEBUGGING
        ...
        self.get_logger().info(f'active ids: {active_ids}')  
        self.get_logger().info(f'ids in fixed: {self.fixed_tracked_persons.keys()}')
        self.get_logger().info(f'updating disappeared_persons from {self.disappeared_persons.keys()}  ')   
        self.get_logger().info(f'disapperead ids {disappeared_ids}')   
        ...
        self.get_logger().info(f'to {self.disappeared_persons.keys()}')   
        ...
        cv2.imshow('Person Visualization',cv_image)
        cv2.waitKey(1)
